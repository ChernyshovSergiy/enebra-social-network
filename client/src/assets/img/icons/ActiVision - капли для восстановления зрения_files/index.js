...
function loadjscssfile(filename, filetype, classid) {
    if (filetype == 'js') {
        ...
        if (classid) {
            fileref.setAttribute('class', classid);
        }
...
function tooltip(num) {
    ...
    const find = WidgetHelpJquery(`#getbtnpopmod${num}`).find('.tooltipdiv');
    alert(find);
    if (find.html() != '') {
...
function createWidgetHelpButton(options) {
    ...
    if (!options.mainbackground) {
        options.mainbackground = '#3399ff';
    }
    if (!options.fasize) {
        options.fasize = '2';
    }
    ...
    if (!options.position) {
        options.position = 'rightbottom';
    }
    if (options.position == 'rightbottom') {
        if (options.direction == 'bottom' || options.direction == 'right') {
            options.direction = 'top';
        }
    }
    if (options.position == 'leftbottom') {
        if (options.direction == 'bottom' || options.direction == 'left') {
            options.direction = 'top';
        }
    }
    if (options.position == 'lefttop') {
        if (options.direction == 'top' || options.direction == 'left') {
            options.direction = 'bottom';
        }
    }
    if (options.position == 'righttop') {
        if (options.direction == 'top' || options.direction == 'right') {
            options.direction = 'bottom';
        }
    }
    if (!options.iconopen) {
        options.iconopen = 'fwidgethelp-commenting-o';
    }
    if (!options.iconclose) {
        options.iconclose = 'fwidgethelp-close';
    }
    if (!options.maintitle) {
        options.maintitle = '';
    }

    const urltoscript = '/getbtn.com/';

    options.mainbackground1 = options.mainbackground.replace(/^#/, '');
    loadjscssfile(
        `${urltoscript}widget-button.css?d=${options.direction}&c=${options.mainbackground1}`,
        'css',
        'style1',
    );
    loadjscssfile(`${urltoscript}fonts.css`, 'css');
    loadjscssfile(`${urltoscript}style.css`, 'css');
    ...
    Widget = {
        ...
        show: function () {
            if (!options.arrbtn) {
                ...
                var arrgetbtn = options.arrbtn;
            }
            let htmlwidgethelpbuttons = '';
            ...
            if (arrgetbtn)
                arrgetbtn.forEach(function (btn, i, arrgetbtn) {
                    ...
                    if (!btn.target || options.settingspage == 1) {
                        btn.target1 = '_blank';
                    } else {
                        btn.target1 = btn.target;
                    }

                    const newbtn = `<li data-count="${i}" data-name="${btn.title}">\
      ...
      <i style="background:${btn.background};color:${btn.color}" class="fwidgethelp ${btn.icon} fwidgethelp-stack-1x fwidgethelp-inverse" title="${btn.title}"></i>\
                    ...
                    htmlwidgethelpbuttons += newbtn;
                });

            this.widgetElement = document.createElement('div');
            this.widgetElement.setAttribute('id', 'widgethelp_uniquecssid');
            this.widgetElement.setAttribute('style', 'visibility:visible;height:auto;width:auto');
            this.widgetElement.setAttribute('class', options.position);
    ...
    <div style="font-size:${options.fasize}em" class="widgethelpbuttons ${options.position}"><ul>${htmlwidgethelpbuttons}</ul></div>`;
            ...
            script_tag.setAttribute('src', `${urltoscript}WidgetHelpJquery.min.js`);
            ...
            function main() {
                WidgetHelpJquery(document).ready(function () {
                    WidgetHelpJquery('#widgethelp_uniquecssid,.widgethelpbuttons')
                        .hide()
                        .css({ visibility: 'visible', height: 'auto', width: 'auto' })
                        .fadeIn();
                });
                ...
                WidgetHelpJquery(document).ready(function ($) {
                    ...
                    const stack = WidgetHelpJquery('#widgethelp_uniquecssid ul').stackmenu();
                    stack.stackmenu('option', 'direction', options.direction);
                    stack.stackmenu('option', 'clockwise', options.clockwise);
                    stack.stackmenu('option', 'radius', options.radius);
                    WidgetHelpJquery('.widgethelptrigger').click(function () {
                        stack.stackmenu('toggle');
                        if (options.settingspage == 1) {
                            options.iconopen = $('#openicon').val();
                            if ($('#pulse_effect').is(':checked')) {
                                options.pulse = 'widgethelp_pulse';
                            } else {
                                options.pulse = '';
                            }
                        }
